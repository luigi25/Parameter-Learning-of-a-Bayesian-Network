...
* La funzione `dataset_gen` prende in ingresso un numero **n** di elementi per determinare la dimensione del data set, la matrice di adiacenza della rete, il vettore dei nodi di questa e l'array delle probabilità creato dalla funzione `probilities_dataset`. La funzione restituisce un data set di dimensione **n**, generando un numero casuale tra **0** e **1** e confrontandolo, nodo per nodo in base all'ordinamento topologico, con le probabilità condizionate presenti. Infatti, vengono differenziati i nodi senza genitori dai nodi con i genitori: nei primi viene solamente eseguito il confronto tra la probabilità che questi accadano e il numero random generato. Nel secondo caso, invece, viene prima controllata la configurazione dei genitori, per determinare la probabilità di accadimento dell'evento in questione e, successivamente, questa viene confrontata con un numero randomico. Il data set è creato, quindi, inserendo i valori **0** e **1** in base al risultato ottenuto dal confronto.
